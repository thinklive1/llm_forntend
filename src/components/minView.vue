...
let models = ref([
  { id: 1, displayName: 'OpenAI GPT-4o', modelId: 'gpt-4o', priority: 1, capabilities: ['text-to-text', 'image-to-text'], status: 'enabled', apiKey: 'sk-...' },
...
<template>
  ...
  <div class="p-6 min-h-screen">
    <header class="flex items-center justify-between pb-4 border-b">
      ...
      <button id="add-new-model-btn" @click="openModal" class="bg-blue-600 text-white font-semibold px-4 py-2 rounded-lg hover:bg-blue-700 flex items-center">
        ...
        <div class="modal-content bg-white rounded-lg shadow-xl w-full max-w-2xl mx-4 p-6 relative">
          ...
          <form id="model-form">
            ...
            <div class="flex justify-end space-x-3">
              <button type="button" @click="closeModal" id="cancel-btn" class="bg-gray-200 text-gray-800 font-semibold px-4 py-2 rounded-lg hover:bg-gray-300">取消</button>
              <button type="submit" @click="closeModal" class="bg-blue-600 text-white font-semibold px-4 py-2 rounded-lg hover:bg-blue-700">保存配置</button>
            </div>
          </form>
        </div>
      </div>
        ...
        <table class="w-full text-sm text-left text-gray-500">
          ...
          <tr v-for="(model, index) in models" :key="index">
            ...
            <td class="px-6 py-4 text-right">
              <button @click="editModel(model)" class="font-medium text-blue-600 hover:underline p-1 ml-2">编辑</button>
              <button @click="deleteModel(model)" class="font-medium text-red-600 hover:underline p-1 ml-2">删除</button>
            </td>
          </tr>
          </tbody>
        </table>
