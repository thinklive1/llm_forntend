...
export default {
  data() {
    return {
      models: [
        { id: 1, name: 'OpenAI GPT-4o', modelId: 'gpt-4o', priority: 1, capabilities: ['text-to-text', 'image-to-text'], status: 'enabled' },
        { id: 2, name: 'Anthropic Claude 3 Sonnet', modelId: 'claude-3-sonnet-20240229', priority: 2, capabilities: ['text-to-text'], status: 'enabled' },
        { id: 3, name: 'OpenAI DALL-E 3', modelId: 'dall-e-3', priority: 1, capabilities: ['text-to-image', 'image-to-image'], status: 'enabled' },
        { id: 4, name: 'Stable Diffusion XL', modelId: 'stable-diffusion-xl-1024-v1-0', priority: 2, capabilities: ['text-to-image', 'image-to-image'], status: 'disabled' },
      ],
  ...
  computed: {
    sortedModels() {
      return this.models.slice().sort((a, b) => a.priority - b.priority || a.name.localeCompare(b.name));
    },
  },
  methods: {
    showModal(model = null) {
      this.isModalVisible = true;
      this.formData = model ? { ...model } : { id: '', name: '', modelId: '', priority: 99, apiKey: '', capabilities: [] };
      this.modalTitle = model ? '编辑模型配置' : '注册新模型';
    },
    hideModal() {
    ...
    submitForm() {
      if (this.formData.id) {
        const index = this.models.findIndex(m => m.id === this.formData.id);
        if (index !== -1) {
          this.models[index] = { ...this.models[index], ...this.formData };
        }
      } else {
        this.formData.id = Date.now(); // Simple unique ID
        this.models.push(this.formData);
      }
      this.hideModal();
    },
    editModel(id) {
    ...
    changePriority(id, direction) {
      ...
      const swapModel = this.models.find(m => m.priority === newPriority);
      model.priority = newPriority;
      if (swapModel) {
        swapModel.priority = oldPriority;
      }
